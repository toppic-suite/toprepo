...
for msalign_fname in "$DIR"/*msalign_info.tsv; do
  ...
  echo "python3 merge_mzml_msalign_toppic_alt_info.py $msalign_basename $feature_fname $toppic_fname toprepo_file_info_v1.2.0.tsv $out"
  python3 merge_mzml_msalign_toppic_alt_info.py $msalign_basename $feature_fname $toppic_fname toprepo_file_info_v1.2.0.tsv $out
