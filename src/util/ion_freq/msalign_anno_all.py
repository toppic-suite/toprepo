...
def match_all_annotations(exp_mass_table, theo_mass_table, seq, ppm_tol=20.0, da_tol=0.01):
    ...
    while i < len(exp_mass_table) and j < len(theo_mass_table):
        ...
        if (
            ...
            if matches:
                ...
                line_out = "\t".join(base_cols + flat_annotations)
            else:
...
def annot_one_spectrum(spectrum, activation_ions=None, ppm_tol=20.0, da_tol=0.01):
    ...
    annot_peak_lines, covered_bonds = match_all_annotations(exp_mass_table, theo_mass_table, seq, ppm_tol=ppm_tol, da_tol=da_tol)
...
def annot_msalign(input_msalign, output_file, activation_ions, ppm_tol=20.0, da_tol=0.01):
    ...
    for spectrum in ms_reader.readmsalign_iter():
        spectrum = annot_one_spectrum(spectrum, activation_ions=activation_ions, ppm_tol=ppm_tol, da_tol=da_tol)
...
if __name__ == "__main__":
    ...
    annot_msalign(args.msalign, output_filename, activation_ions, ppm_tol, da_tol)    